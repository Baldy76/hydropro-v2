...
window.printReceipt = (id) => {
    ...
    win.document.write(`<html><body style="font-family:sans-serif;text-align:center;padding:50px;"><h1>HYDRO PRO</h1><p>PAID IN FULL</p><hr><p>Customer: ${c.name}</p><p>Amount: £${n(c.price).toFixed(2)}</p><script>window.onload=function(){window.print();}</script></body></html>`);
    win.document.close();
};
...
window.renderWeek = () => {
    ...
    if(jobs.length === 0) { list.innerHTML = '<p style="text-align:center; padding:100px 0; opacity:0.3; font-weight:900;">NO JOBS TODAY</p>'; return; }
    jobs.forEach(c => {
        const div = document.createElement('div'); div.className = 'JOB-CARD';
        div.onclick = () => showJobBriefing(c.id);
        div.innerHTML = `
            <div style="text-align:left;"><strong style="font-size:24px;">${c.name} ${c.cleaned?'✅':''}</strong><br><small style="color:var(--accent); font-weight:800; font-size:18px;">${c.houseNum} ${c.street}</small></div>
...
window.showJobBriefing = (id) => {
    ...
    let htm = ''; history.forEach(h => { htm += `<div style="display:flex; justify-content:space-between; padding:10px 0; border-bottom:1px solid rgba(0,0,0,0.05); font-weight:800; font-size:18px;"><span>${h.date}</span><span style="color:var(--success)">+£${n(h.amt).toFixed(2)}</span></div>`; });
    document.getElementById('briefingData').innerHTML = `<div style="font-size:40px; font-weight:900; color:var(--accent);">${c.name}</div><div style="margin:30px 0; display:flex; flex-direction:column; gap:15px;"><div style="background:var(--bg); padding:25px; border-radius:25px; font-weight:800; font-size:20px;">🏠 ${c.houseNum} ${c.street}</div><div style="background:var(--bg); padding:25px; border-radius:25px; font-weight:800; font-size:20px; color:var(--success);">💰 Rate: £${n(c.price).toFixed(2)}</div></div><div class="MD-HIST-WRAP"><small style="text-transform:uppercase; font-weight:900; opacity:0.4; font-size:12px;">Payment History</small>${htm || '<p>No history</p>'}</div>`;
    document.getElementById('receiptActionBox').innerHTML = n(c.paidThisMonth) > 0 ? `<button class="BTN-RECEIPT-TEXT" onclick="sendSmsReceipt('${c.id}')">💬 TEXT RECEIPT</button><button class="BTN-RECEIPT-PRINT" onclick="printReceipt('${c.id}')">🖨️ PRINT / PDF</button>` : '';
...
window.exportData = () => {
    ...
    const l = document.createElement("a"); l.href = u; l.download = `HP_MASTER_${d}.csv`; l.click();
};
...
window.importData = (event) => {
    ...
    r.onload = (e) => {
        ...
        let s = ""; lines.forEach(line => {
            ...
            if(trim.includes("--- EXPENSES ---")) { s = "E"; return; }
            if(trim.includes("--- HISTORY ---")) { s = "H"; return; }
            ...
            else if(s === "E" && cols.length >= 4) { db.expenses.push({ date: cl(cols[0]), cat: cl(cols[1]), desc: cl(cols[2]), amt: n(cl(cols[3])) }); }
            else if(s === "H" && cols.length >= 3) { db.history.push({ date: cl(cols[0]), custId: cl(cols[1]), amt: n(cl(cols[2])) }); }
        });
