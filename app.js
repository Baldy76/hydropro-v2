...
window.renderStats = () => {
    ...
    container.innerHTML = `
        ...
        ${arrears > 0 ? `<div class="arrears-banner"><small style="display:block;font-size:11px;opacity:0.8;">ARREARS DETECTED</small>£${arrears.toFixed(2)}</div>` : ''}
    `;
};
...
window.showJobBriefing = (id) => {
    ...
    document.getElementById('briefingData').innerHTML = `
        ...
        <div class="briefing-meta">📍 ${c.postcode} | 📱 ${c.phone}</div>
        <div class="briefing-item"><h4>Current Rate</h4><div style="font-size:22px; font-weight:900;">£${n(c.price).toFixed(2)}</div></div>
        ${currentOwed > 0 ? `<div class="briefing-item" style="border:2px solid var(--danger); background:rgba(255,69,58,0.05);"><h4>Arrears This Month</h4><div style="color:var(--danger); font-size:22px; font-weight:900;">£${currentOwed.toFixed(2)}</div></div>` : ''}
        <div class="briefing-item"><h4>Last 3 Payments</h4>${histHtml}</div>
        <div class="briefing-item"><h4>Private Notes</h4><div style="font-size:14px; line-height:1.4;">${c.notes || 'No notes saved.'}</div></div>
    `;
