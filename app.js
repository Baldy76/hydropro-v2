...
window.onload = () => {
    ...
    // Check Preference & Adaptive Logo
...
const fetchWeather = async (lat, lon) => {
    try {
        ...
        document.getElementById('wIcon').innerText = data.current_weather.weathercode <= 3 ? "☀️" : "☁️";
    } catch (e) {}
};
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        weekCusts.forEach(c => {
            ...
            card.innerHTML = `<div onclick="showCustDetails('${c.id}')"><strong style="font-size:19px; color:var(--accent);">${c.name}</strong><br><small style="opacity:0.6;">${c.address}</small></div>
                ...
                <div class="status-row" style="display:grid; gap:8px; grid-template-columns:${d > 0 ? 'repeat(3,1fr)' : '1fr 1fr'}"><button class="action-btn-main bounce-on-tap ${c.cleaned ? 'btn-cleaned-active' : ''}" onclick="toggleCleaned('${c.id}')">${c.cleaned ? 'Done' : 'Cleaned'}</button><button class="action-btn-main bounce-on-tap ${isPaid ? 'btn-paid-active' : 'btn-pay-pending'}" onclick="markAsPaid('${c.id}')">${isPaid ? 'Paid' : 'Pay £'+n(c.price)}</button>${d > 0 ? `<button class="action-btn-main bounce-on-tap" onclick="handleDebtCollection('${c.id}')" style="background:var(--danger); color:white;">Debt £${d}</button>` : ''}</div></div>`;
