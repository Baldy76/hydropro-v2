...
function migrateDays() { db.customers.forEach(c => { if(!c.day) c.day = 'Mon'; }); }
...
window.renderStats = () => {
    ...
    container.innerHTML = `<div class="stats-hero"><div>£${profit.toFixed(2)}</div><small>NET PROFIT</small></div><div class="stats-split-grid"><div class="stats-card"><span>📈</span><small>INCOME</small><strong style="color:var(--success);">£${paid.toFixed(2)}</strong></div><div class="stats-card"><span>📉</span><small>SPEND</small><strong style="color:var(--danger);">£${spend.toFixed(2)}</strong></div></div><div class="stats-progress-card"><div style="display:flex;justify-content:space-between;font-weight:900;font-size:13px;"><span>PROGRESS</span><span>${progress}%</span></div><div class="progress-bar-track"><div class="progress-bar-fill" style="width:${progress}%"></div></div></div>${arrears > 0 ? `<div class="arrears-banner"><small>ARREARS</small><br>£${arrears.toFixed(2)}</div>` : ''}`;
};
...
window.showJobBriefing = (id) => {
    ...
    document.getElementById('briefingData').innerHTML = `<div class="briefing-title">${c.name}</div><div class="briefing-addr">${c.houseNum} ${c.street}</div><div style="font-size:14px; opacity:0.5; margin-bottom:20px; font-weight:600;">📍 ${c.postcode} | 📱 ${c.phone} | 📅 ${c.day}s</div><div class="briefing-item"><h4>Current Rate</h4><div style="font-size:22px; font-weight:900;">£${n(c.price).toFixed(2)}</div></div>${currentOwed > 0 ? `<div class="briefing-item" style="border:2px solid var(--danger); background:rgba(255,69,58,0.05);"><h4>Arrears</h4><div style="color:var(--danger); font-size:22px; font-weight:900;">£${currentOwed.toFixed(2)}</div></div>` : ''}<div class="briefing-item"><h4>Last 3 Payments</h4>${histHtml}</div><div class="briefing-item"><h4>Private Notes</h4><div style="font-size:14px; line-height:1.4;">${c.notes || 'No notes saved.'}</div></div>`;
...
window.renderWeek = () => {
    ...
    db.customers.filter(c => c.week == curWeek && c.day == workingDay).forEach(c => {
        ...
        let badge = (c.cleaned && n(c.paidThisMonth) < n(c.price)) ? `<div class="arrears-badge">UNPAID 🚩</div>` : "";
        div.innerHTML = `${badge}<div><strong style="font-size:20px;">${c.name} ${c.cleaned?'✅':''}</strong><br><small style="color:var(--accent); font-weight:700;">${c.houseNum} ${c.street}</small></div><div class="job-actions" onclick="event.stopPropagation()"><button class="btn-job-control btn-map-yellow" onclick="openMap('${c.houseNum} ${c.street} ${c.postcode}')">📍 MAP</button><button class="btn-job-control" style="background:${c.cleaned?'var(--success)':'#aaa'}" onclick="handleClean('${c.id}')">CLEAN</button><button class="btn-job-control" style="background:${n(c.paidThisMonth)>0?'var(--accent)':'#aaa'}" onclick="markAsPaid('${c.id}')">PAY</button></div>`;
...
window.markAsPaid = (id) => {
    ...
    if(amt !== null) { c.paidThisMonth = n(amt); db.history.push({ custId: id, amt: n(amt), date: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) }); saveData(); renderWeek(); renderStats(); }
};
window.renderLedger = () => {
