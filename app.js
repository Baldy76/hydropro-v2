...
/* --- 👥 MASTER LIST & FULL PROFILE MODAL --- */
window.renderMaster = () => {
    ...
    db.customers.forEach(c => {
        if(c.name.toLowerCase().includes(search) || (c.street||"").toLowerCase().includes(search)) {
            ...
            div.innerHTML = `
                ...
                <div style="font-weight:900; color:var(--success); font-size:26px;">£${n(c.price).toFixed(2)}</div>`;
...
window.renderStats = () => {
    ...
    container.innerHTML = `<div class="ST-HERO"><div>£${(collected - (fuel+gear+food+misc)).toFixed(2)}</div><small>NET PROFIT</small></div><div class="ST-GRID"><div class="ST-BUBBLE">INCOME<strong>£${collected.toFixed(2)}</strong></div><div class="ST-BUBBLE">SPEND<strong>£${(fuel+gear+food+misc).toFixed(2)}</strong></div></div><div class="ST-PROG-CARD"><div style="display:flex; justify-content:space-between; font-weight:900;"><span>PROGRESS</span><span>${prog}%</span></div><div class="ST-PROG-TRACK"><div class="ST-PROG-FILL" style="width:${prog}%"></div></div></div>`;
};
