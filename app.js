...
window.openEditModal = (id) => {
    ...
    document.getElementById('eDay').value = c.day || 'Mon';
    document.getElementById('eWeek').value = c.week || '1';
...
window.showJobBriefing = (id) => {
    ...
    document.getElementById('briefingData').innerHTML = `<div style="font-size:40px; font-weight:900; color:var(--accent);">${c.name}</div><div style="margin:30px 0; display:flex; flex-direction:column; gap:12px;"><div style="background:var(--bg); padding:25px; border-radius:25px; font-weight:800; font-size:20px;">🏠 ${c.houseNum} ${c.street}</div><div style="background:var(--bg); padding:25px; border-radius:25px; font-weight:800; font-size:20px; color:var(--success);">💰 Rate: £${n(c.price).toFixed(2)}</div></div><div class="MD-HIST-WRAP"><small style="text-transform:uppercase; font-weight:900; opacity:0.4; font-size:12px;">Recent Payments</small>${htm || '<p>No history</p>'}</div>`;
    document.getElementById('receiptActionBox').innerHTML = n(c.paidThisMonth) > 0 ? `<button style="height:70px; background:#5856d6; color:white; border:none; border-radius:20px; font-weight:900; font-size:18px; margin-bottom:15px;" onclick="sendSmsReceipt('${c.id}')">💬 TEXT RECEIPT</button><button style="height:70px; background:var(--ios-grey); color:var(--text); border:none; border-radius:20px; font-weight:900; font-size:18px; margin-bottom:15px;" onclick="printReceipt('${c.id}')">🖨️ PRINT / PDF</button>` : '';
