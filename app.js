...
window.openTab = (name) => {
    ...
    if (header) header.style.display = name === 'home' ? "flex" : "none"; // Hide logo header in tools to save space
...
window.saveCustomer = () => {
    const nameVal = document.getElementById('cName').value; if(!nameVal) return;
    ...
    const entry = {
        ...
        cleaned: ex ? ex.cleaned : false, paidThisMonth: ex ? ex.paidThisMonth : 0, debtHistory: ex ? ex.debtHistory : [], paymentLogs: ex ? ex.paymentLogs : []
    };
...
window.renderWeekLists = () => {
    for (let i = 1; i <= 5; i++) {
        ...
        if (weekCusts.length === 0) { container.innerHTML += `<div class="card" style="text-align:center; opacity:0.5; padding:60px;">Week ${i} is empty</div>`; continue; }
        weekCusts.forEach(c => {
