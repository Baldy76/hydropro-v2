...
/* --- 🌦️ WEATHER (v38.7 audit: secure endpoint) --- */
...
/* --- 📄 SMART RECEIPT ENGINE (v38.7 audit: non-invasive) --- */
...
window.printReceipt = (id) => {
    ...
    win.document.write(`
        <html><head><style>body{font-family:sans-serif;text-align:center;padding:50px;} .paid{border:4px solid green; color:green; display:inline-block; padding:10px 20px; font-weight:900; transform:rotate(-5deg); font-size:24px; margin:20px 0; border-radius:10px;}</style></head>
        <body><h1>HYDRO PRO</h1><div class="paid">PAID IN FULL</div><hr><p>Customer: ${c.name}</p><p>Amount: £${n(c.price).toFixed(2)}</p><p>Date: ${new Date().toLocaleDateString()}</p>
...
window.renderWeek = () => {
    ...
    if(jobs.length === 0) { list.innerHTML = '<p style="text-align:center; padding:100px 0; opacity:0.3; font-weight:900; font-size:20px;">NO JOBS TODAY</p>'; return; }
    jobs.forEach(c => {
        const div = document.createElement('div'); div.className = 'JOB-CARD';
        div.onclick = () => showJobBriefing(c.id);
        div.innerHTML = `
            <div style="text-align:left;">
                <strong style="font-size:24px;">${c.name} ${c.cleaned?'✅':''}</strong><br>
                <small style="color:var(--accent); font-weight:800; font-size:18px;">${c.houseNum} ${c.street}</small>
            </div>
...
window.showJobBriefing = (id) => {
    ...
    let htm = ''; history.forEach(h => { htm += `<div style="display:flex; justify-content:space-between; padding:12px 0; border-bottom:1px solid rgba(0,0,0,0.05); font-weight:800; font-size:18px;"><span>${h.date}</span><span style="color:var(--success)">+£${n(h.amt).toFixed(2)}</span></div>`; });
    
    document.getElementById('briefingData').innerHTML = `
        <div style="font-size:40px; font-weight:900; color:var(--accent);">${c.name}</div>
        <div style="margin:30px 0; display:flex; flex-direction:column; gap:15px;">
            <div style="background:var(--bg); padding:25px; border-radius:25px; font-weight:800; font-size:20px;">🏠 ${c.houseNum} ${c.street}</div>
            <div style="background:var(--bg); padding:25px; border-radius:25px; font-weight:800; font-size:20px; color:var(--success);">💰 Rate: £${n(c.price).toFixed(2)}</div>
        </div>
        <div class="MD-HIST-WRAP"><small style="text-transform:uppercase; font-weight:900; opacity:0.4; font-size:12px;">Payment History</small>${htm || '<p>No history</p>'}</div>
    `;
    
    document.getElementById('receiptActionBox').innerHTML = n(c.paidThisMonth) > 0 ? `
        <button class="BTN-RECEIPT-TEXT" onclick="sendSmsReceipt('${c.id}')">💬 TEXT RECEIPT</button>
        <button class="BTN-RECEIPT-PRINT" onclick="printReceipt('${c.id}')">🖨️ PRINT / PDF</button>
    ` : '';
...
window.renderLedger = () => { const container = document.getElementById('ledger-list-container'); if(!container) return; container.innerHTML = ''; let total = 0; db.expenses.forEach(e => total += n(e.amt)); document.getElementById('ledgerTotalDisplay').innerText = `£${total.toFixed(2)}`; db.expenses.slice().reverse().forEach(e => { const div = document.createElement('div'); div.className = 'LD-PILL'; div.innerHTML = `<div style="text-align:left;"><strong>${e.cat||'📦'} ${e.desc}</strong><br><small>${e.date}</small></div><div style="color:var(--danger); font-weight:900; font-size:20px;">-£${n(e.amt).toFixed(2)}</div>`; container.appendChild(div); }); };
window.renderStats = () => { const container = document.getElementById('stats-container'); if(!container) return; let t = 0, p = 0, s = 0; db.customers.forEach(c => { t += n(c.price); p += n(c.paidThisMonth); }); db.expenses.forEach(e => s += n(e.amt)); const prog = t > 0 ? Math.min(Math.round((p / t) * 100), 100) : 0; container.innerHTML = `<div class="ST-HERO"><div>£${(p - s).toFixed(2)}</div><small>NET PROFIT</small></div><div class="ST-GRID"><div class="ST-BUBBLE">INCOME<strong>£${p.toFixed(2)}</strong></div><div class="ST-BUBBLE">SPEND<strong>£${s.toFixed(2)}</strong></div></div><div class="ST-PROG-CARD" style="padding:30px; border-radius:40px; background:var(--card); margin:0 30px 30px;"><strong>Round Target: ${prog}%</strong><div style="background:var(--bg); height:20px; border-radius:15px; margin-top:15px; overflow:hidden;"><div style="background:var(--accent); width:${prog}%; height:100%; transition:1s;"></div></div></div>`; };
window.renderMaster = () => { const container = document.getElementById('master-list-container'); if(!container) return; container.innerHTML = ''; const search = (document.getElementById('mainSearch').value || "").toLowerCase(); db.customers.forEach(c => { if(c.name.toLowerCase().includes(search) || (c.street||"").toLowerCase().includes(search)) { const div = document.createElement('div'); div.className = 'CT-PILL'; div.onclick = () => editCust(c.id); div.innerHTML = `<div class="CT-TEXT-STACK"><strong>${c.name}</strong><small>${c.houseNum} ${c.street}</small></div><div style="font-weight:900; color:var(--success); font-size:26px;">£${n(c.price).toFixed(2)}</div>`; container.appendChild(div); } }); };
