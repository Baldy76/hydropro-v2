...
/* --- (Rest of standard Master/Stats/Ledger logic preserved) --- */
window.renderMaster = () => { const container = document.getElementById('master-list-container'); if(!container) return; container.innerHTML = ''; const search = (document.getElementById('mainSearch').value || "").toLowerCase(); db.customers.forEach(c => { if(c.name.toLowerCase().includes(search) || (c.street||"").toLowerCase().includes(search)) { const div = document.createElement('div'); div.className = 'CT-PILL'; div.onclick = () => openEditModal(c.id); div.innerHTML = `<div class="CT-TEXT-STACK"><strong>${c.name}</strong><small>${c.houseNum} ${c.street}</small></div><div style="font-weight:900; color:var(--success); font-size:26px;">£${n(c.price).toFixed(2)}</div>`; container.appendChild(div); } }); };
window.openEditModal = (id) => { const c = db.customers.find(x => x.id === id); if(!c) return; document.getElementById('eEditId').value = c.id; document.getElementById('eName').value = c.name; document.getElementById('ePhone').value = c.phone || ''; document.getElementById('eHouseNum').value = c.houseNum || ''; document.getElementById('eStreet').value = c.street || ''; document.getElementById('ePostcode').value = c.postcode || ''; document.getElementById('eDay').value = c.day || 'Mon'; document.getElementById('eWeek').value = c.week || '1'; document.getElementById('ePrice').value = c.price || 0; document.getElementById('eNotes').value = c.notes || ''; document.getElementById('editModal').classList.remove('hidden'); };
window.updateCustomerFromModal = () => { const id = document.getElementById('eEditId').value; const idx = db.customers.findIndex(c => c.id === id); if(idx > -1) { db.customers[idx].name = document.getElementById('eName').value; db.customers[idx].phone = document.getElementById('ePhone').value; db.customers[idx].houseNum = document.getElementById('eHouseNum').value; db.customers[idx].street = document.getElementById('eStreet').value; db.customers[idx].postcode = document.getElementById('ePostcode').value.toUpperCase(); db.customers[idx].day = document.getElementById('eDay').value; db.customers[idx].week = document.getElementById('eWeek').value; db.customers[idx].price = n(document.getElementById('ePrice').value); db.customers[idx].notes = document.getElementById('eNotes').value; saveData(); closeEditModal(); renderMaster(); } };
window.renderStats = () => { const container = document.getElementById('stats-container'); if(!container) return; let t = 0, coll = 0, fuel = 0, gear = 0, food = 0, misc = 0; db.customers.forEach(c => { t += n(c.price); coll += n(c.paidThisMonth); }); db.expenses.forEach(e => { const cat = (e.cat||"").toLowerCase(); if(cat.includes('fuel')) fuel += n(e.amt); else if(cat.includes('gear')) gear += n(e.amt); else if(cat.includes('food')) food += n(e.amt); else misc += n(e.amt); }); const spend = fuel + gear + food + misc; const prog = t > 0 ? Math.min(Math.round((coll / t) * 100), 100) : 0; container.innerHTML = `<div class="ST-HERO"><div>£${(coll - spend).toFixed(2)}</div><small>NET PROFIT</small></div><div class="ST-GRID"><div class="ST-BUBBLE">INCOME<strong>£${coll.toFixed(2)}</strong></div><div class="ST-BUBBLE">SPEND<strong>£${spend.toFixed(2)}</strong></div></div><div class="ST-PROG-CARD" style="padding:30px; border-radius:40px; background:var(--card); margin:0 30px 30px;"><strong>Progress: ${prog}%</strong><div style="background:var(--bg); height:20px; border-radius:15px; margin-top:15px; overflow:hidden;"><div style="background:var(--accent); width:${prog}%; height:100%; transition:1s;"></div></div></div><div class="SU-CARD" style="margin:0 30px;"><div style="display:flex; justify-content:space-between; padding:12px 0; border-bottom:1px solid rgba(0,0,0,0.05); font-weight:800;"><span>⛽ Fuel</span><span>£${fuel.toFixed(2)}</span></div><div style="display:flex; justify-content:space-between; padding:12px 0; border-bottom:1px solid rgba(0,0,0,0.05); font-weight:800;"><span>🛠️ Gear</span><span>£${gear.toFixed(2)}</span></div><div style="display:flex; justify-content:space-between; padding:12px 0; font-weight:800;"><span>📦 Misc</span><span>£${misc.toFixed(2)}</span></div></div>`; };
...
window.showJobBriefing = (id) => { const c = db.customers.find(x => x.id === id); if(!c) return; const history = (db.history || []).filter(h => h.custId === id).slice(-3).reverse(); let htm = ''; history.forEach(h => { htm += `<div style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid rgba(0,0,0,0.05); font-weight:800; font-size:16px;"><span>${h.date}</span><span style="color:var(--success)">+£${n(h.amt).toFixed(2)}</span></div>`; }); document.getElementById('briefingData').innerHTML = `<div style="font-size:40px; font-weight:900; color:var(--accent);">${c.name}</div><div style="margin:30px 0; display:flex; flex-direction:column; gap:12px;"><div style="background:var(--bg); padding:25px; border-radius:25px; font-weight:800; font-size:20px;">🏠 ${c.houseNum} ${c.street}</div><div style="background:var(--bg); padding:25px; border-radius:25px; font-weight:800; font-size:20px; color:var(--success);">💰 Rate: £${n(c.price).toFixed(2)}</div></div><div class="MD-HIST-WRAP"><small style="text-transform:uppercase; font-weight:900; opacity:0.4; font-size:12px;">Payment History</small>${htm || '<p>No history</p>'}</div>`; document.getElementById('receiptActionBox').innerHTML = n(c.paidThisMonth) > 0 ? `<button style="height:70px; background:#5856d6; color:white; border:none; border-radius:20px; font-weight:900; font-size:18px; margin-bottom:15px;" onclick="sendSmsReceipt('${c.id}')">💬 TEXT RECEIPT</button><button style="height:70px; background:var(--ios-grey); color:var(--text); border:none; border-radius:20px; font-weight:900; font-size:18px; margin-bottom:15px;" onclick="printReceipt('${c.id}')">🖨️ PRINT / PDF</button>` : ''; document.getElementById('briefEditBtn').onclick = () => { closeBriefing(); openEditModal(id); }; document.getElementById('briefingModal').classList.remove('hidden'); };
window.sendSmsReceipt = (id) => { const c = db.customers.find(x => x.id === id); if(!c) return; const msg = encodeURIComponent(`Hi ${c.name}, thanks for your payment of £${n(c.price).toFixed(2)} to Hydro Pro. Your windows are all sparkling! 🧼`); window.location.href = `sms:${c.phone}?&body=${msg}`; };
window.printReceipt = (id) => { const c = db.customers.find(x => x.id === id); if(!c) return; const win = window.open('', '_blank'); win.document.write(`<html><body style="font-family:sans-serif;text-align:center;padding:50px;"><h1>HYDRO PRO</h1><p>PAID IN FULL</p><hr><p>Customer: ${c.name}</p><p>Amount: £${n(c.price).toFixed(2)}</p><script>window.onload=function(){window.print();}</script></body></html>`); win.document.close(); };
...
window.completeCycle = () => { if(confirm("Clear month?")) { db.customers.forEach(c => { c.cleaned = false; c.paidThisMonth = 0; }); saveData(); location.reload(); } };
