...
window.renderWeekLists = () => {
    for (let i = 1; i <= 5; i++) {
        ...
        db.customers.filter(c => c.week == i).forEach(c => {
            const card = document.createElement('div'); card.className = 'customer-pill-bubble'; card.style.flexDirection = 'column';
            card.innerHTML = `
                <div style="display:flex; justify-content:space-between; width:100%; align-items:center;">
                    <div onclick="showActionModal('${c.id}')"><strong style="font-size:22px;">${c.name} ${c.cleaned ? '✅' : ''}</strong><br><small>${c.houseNum} ${c.street}</small></div>
                    <button class="bank-unlock-ironclad" style="width:50px; height:50px; background:#007aff" onclick="sendReminder('${c.id}', 'whatsapp')">WA</button>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; margin-top:20px; width:100%">
                    <button class="bank-unlock-ironclad" style="width:100%; ${c.cleaned ? 'background:var(--success)' : 'background:#aaa'}" onclick="toggleCleaned('${c.id}')">Clean</button>
                    <button class="bank-unlock-ironclad" style="width:100%; ${n(c.paidThisMonth)>0 ? 'background:var(--accent)' : 'background:#aaa'}" onclick="markAsPaid('${c.id}')">Pay</button>
                    <button class="bank-unlock-ironclad" style="width:100%; background:#777" onclick="editCust('${c.id}')">Edit</button>
                </div>`;
...
window.messageAll = (weekNum, method) => {
    ...
    list.forEach((c, index) => {
        setTimeout(() => {
            const msg = `Hi ${c.name}, it's Hydro Pro. Reminder: I'm coming to clean your windows this week!`;
...
window.sendReminder = (id, method) => {
    const c = db.customers.find(x => x.id === id); if(!c || !c.phone) return alert("Add phone first!");
    const msg = `Hi ${c.name}, it's Hydro Pro. Cleaning windows today!`;
    window.open(`https://wa.me/${c.phone.replace(/\s+/g, '')}?text=${encodeURIComponent(msg)}`, '_blank');
};
...
window.renderLedger = () => { const list = document.getElementById('expenseList'); if(!list) return; list.innerHTML = ''; db.expenses.slice().reverse().forEach(e => { const div = document.createElement('div'); div.className = 'customer-pill-bubble'; div.innerHTML = `<div><strong>${e.desc}</strong><br><small>${e.date}</small></div><div style="color:var(--danger); font-weight:900">-£${n(e.amt).toFixed(2)}</div>`; list.appendChild(div); }); };
