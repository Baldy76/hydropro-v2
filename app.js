...
window.renderStats = () => {
    ...
    container.innerHTML = `
        <div class="stats-hero-main"><div>£${profit.toFixed(2)}</div><small>PROFIT</small></div>
        <div class="stats-grid-row">
            <div class="stats-card-heavy"><span>📈</span><small>INCOME</small><div class="stats-value-large" style="color:var(--success);">£${paid.toFixed(2)}</div></div>
            <div class="stats-card-heavy"><span>📉</span><small>SPEND</small><div class="stats-value-large" style="color:var(--danger);">£${spend.toFixed(2)}</div></div>
        </div>
        ...
        ${arrears > 0 ? `<div class="stats-arrears-banner"><small>ARREARS</small><br>£${arrears.toFixed(2)}</div>` : ''}
    `;
};
...
window.showJobBriefing = (id) => {
    ...
    history.forEach(h => histHtml += `<div style="display:flex; justify-content:space-between; padding:5px 0; font-size:14px; border-bottom:1px solid rgba(0,0,0,0.05);"><span>${h.date}</span><span style="color:var(--success)">+£${n(h.amt).toFixed(2)}</span></div>`);
    ...
    document.getElementById('briefingData').innerHTML = `
        ...
        <div style="font-size:14px; opacity:0.6; margin-bottom:20px;">📍 ${c.postcode}<br>📱 ${c.phone}</div>
        <div class="brief-section"><h4>Rate</h4><div style="font-size:22px; font-weight:900;">£${n(c.price).toFixed(2)}</div></div>
        <div class="brief-section"><h4>Payment History</h4>${histHtml}</div>
        <div class="brief-section"><h4>Notes</h4><div>${c.notes || 'No notes.'}</div></div>
    `;
...
window.renderWeek = () => {
    ...
    bulk.innerHTML = `<button style="background:#25d366;color:white;height:65px;width:100%;border-radius:20px;border:none;font-weight:900;margin-bottom:12px;" onclick="messageAll(${curWeek},'wa')">WhatsApp W${curWeek}</button><button style="background:#007aff;color:white;height:65px;width:100%;border-radius:20px;border:none;font-weight:900;" onclick="messageAll(${curWeek},'sms')">SMS W${curWeek}</button>`;
    list.innerHTML = '';
