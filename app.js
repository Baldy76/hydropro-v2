...
window.renderStats = () => {
    ...
    container.innerHTML = `
        ...
        <div class="stats-grid">
            <div class="stats-card"><small>INCOME</small><div style="color:var(--success);">£${paid.toFixed(2)}</div></div>
            <div class="stats-card"><small>SPEND</small><div style="color:var(--danger);">£${spend.toFixed(2)}</div></div>
        </div>
        <div class="stats-progress">
            <div style="display:flex;justify-content:space-between;font-weight:900;font-size:12px;"><span>PROGRESS</span><span>${progress}%</span></div>
            <div class="progress-bg"><div class="progress-fill" style="width:${progress}%"></div></div>
            <div style="display:flex;justify-content:space-between;font-size:10px;opacity:0.5;font-weight:800;"><span>GOAL £${target.toFixed(2)}</span><span>OWED £${(target - paid).toFixed(2)}</span></div>
        </div>
        ${arrears > 0 ? `<div class="stats-alert"><small>ARREARS</small><br>£${arrears.toFixed(2)}</div>` : ''}
    `;
};
...
window.renderWeek = () => { const bulk = document.getElementById('bulk-box'), list = document.getElementById('week-list-container'); if(!list) return; bulk.innerHTML = `<button class="btn-wa" style="background:#25d366;color:white;height:60px;width:100%;border-radius:15px;border:none;font-weight:900;margin-bottom:10px;" onclick="messageAll(${curWeek},'wa')">WA Message All W${curWeek}</button><button class="btn-sms" style="background:#007aff;color:white;height:60px;width:100%;border-radius:15px;border:none;font-weight:900;" onclick="messageAll(${curWeek},'sms')">SMS Message All W${curWeek}</button>`; list.innerHTML = ''; db.customers.filter(c => c.week == curWeek).forEach(c => { const div = document.createElement('div'); div.className = 'job-card'; div.innerHTML = `<div><strong style="font-size:18px;">${c.name} ${c.cleaned?'✅':''}</strong><br><small style="color:var(--accent); font-weight:700;">${c.houseNum} ${c.street}</small></div><div class="job-actions"><button class="btn-job btn-map-yellow" onclick="openMap('${c.houseNum} ${c.street} ${c.postcode}')">📍 MAP</button><button class="btn-job" style="background:${c.cleaned?'var(--success)':'#aaa'}" onclick="handleClean('${c.id}')">CLEAN</button><button class="btn-job" style="background:${n(c.paidThisMonth)>0?'var(--accent)':'#aaa'}" onclick="markAsPaid('${c.id}')">PAY</button></div>`; list.appendChild(div); }); };
window.markAsPaid = (id) => { const c = db.customers.find(x => x.id === id); if(!c) return; if(n(c.paidThisMonth) > 0) { if(confirm(`Paid £${n(c.paidThisMonth)}. Reset?`)) { c.paidThisMonth = 0; saveData(); renderWeek(); renderStats(); } return; } const amt = prompt(`Paid for ${c.name}:`, c.price); if(amt !== null) { c.paidThisMonth = n(amt); saveData(); renderWeek(); renderStats(); } };
