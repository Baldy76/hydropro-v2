        ...
        container.innerHTML = `
            <div class="iron-spaced-stack">
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:10px;">
                    <button class="tile-restore" style="height:65px; font-size:15px; background:#25d366; color:white" onclick="messageAll(${i}, 'whatsapp')">WA ALL</button>
                    <button class="tile-restore" style="height:65px; font-size:15px; background:#ff9500; color:white" onclick="messageAll(${i}, 'sms')">SMS ALL</button>
                    <button class="tile-restore" style="height:65px; font-size:15px; background:#e5e5ea; color:#000" onclick="openTab('weeksHub')">⬅️ Hub</button>
                    <button class="tile-restore" style="height:65px; font-size:15px; background:#e5e5ea; color:#000" onclick="openTab('home')">🏠 Home</button>
                </div>
        ...
        db.customers.filter(c => c.week == i).forEach(c => {
            ...
            card.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <button class="bank-toggle-btn" style="background:#007aff; color:white; padding:10px; border-radius:10px; border:none;" onclick="sendReminder('${c.id}', 'whatsapp')">WA</button>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; margin-top:15px;">
                    <button class="bank-toggle-btn" style="height:44px; border:none; border-radius:10px; ${c.cleaned ? 'background:var(--success); color:white;' : ''}" onclick="toggleCleaned('${c.id}')">Clean</button>
                    <button class="bank-toggle-btn" style="height:44px; border:none; border-radius:10px; ${n(c.paidThisMonth)>0 ? 'background:var(--accent); color:white;' : ''}" onclick="markAsPaid('${c.id}')">Pay</button>
                    <button class="bank-toggle-btn" style="height:44px; border:none; border-radius:10px;" onclick="showActionModal('${c.id}')">Edit</button>
                </div>`;
