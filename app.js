...
window.renderWeek = () => {
    ...
    jobs.forEach(c => {
        ...
        div.style = "background:var(--card); padding:25px; border-radius:40px; margin:0 25px 15px; display:flex; justify-content:space-between; align-items:center; box-shadow:0 8px 15px rgba(0,0,0,0.03);";
        div.onclick = () => showJobBriefing(c.id);
        div.innerHTML = `<div><strong style="font-size:20px;">${c.name} ${c.cleaned?'✅':''}</strong><br><small style="font-weight:800; color:var(--accent); text-transform:uppercase;">${c.houseNum} ${c.street}</small></div><div style="font-weight:900; font-size:20px;">£${n(c.price).toFixed(2)}</div>`;
...
window.renderMaster = () => {
    ...
    db.customers.forEach(c => {
        if(c.name.toLowerCase().includes(search) || (c.street||"").toLowerCase().includes(search)) {
            const div = document.createElement('div');
            div.style = "background:var(--card); padding:25px 30px; border-radius:40px; display:flex; justify-content:space-between; align-items:center; margin:0 25px 15px; height:120px !important; box-shadow:0 8px 15px rgba(0,0,0,0.04);";
