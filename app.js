...
window.renderAll = () => {
    ...
    renderWeekFence();
    if(db.bank) {
        const bn = document.getElementById('bankName'); if(bn) bn.value = db.bank.name || '';
        const bs = document.getElementById('bankSort'); if(bs) bs.value = db.bank.sort || '';
        const ba = document.getElementById('bankAcc'); if(ba) ba.value = db.bank.acc || '';
    }
};
...
window.renderWeekFence = () => {
    ...
    if(!list) return;
    bulk.innerHTML = `<button class="btn-bulk btn-wa" onclick="messageAll(${currentActiveWeek}, 'whatsapp')">WA ALL W${currentActiveWeek}</button><button class="btn-bulk btn-sms" onclick="messageAll(${currentActiveWeek}, 'sms')">SMS ALL W${currentActiveWeek}</button>`;
    list.innerHTML = '';
    db.customers.filter(c => c.week == currentActiveWeek).forEach(c => {
        ...
        div.innerHTML = `<div><span style="font-size:22px; font-weight:800;">${c.name} ${c.cleaned ? '✅' : ''}</span><br><span style="color:var(--accent); font-weight:700;">${c.houseNum} ${c.street}</span></div>
            <div class="job-action-grid">
                <button class="btn-job" style="background:${c.cleaned ? 'var(--success)' : '#aaa'}" onclick="toggleCleaned('${c.id}')">CLEAN</button>
                <button class="btn-job" style="background:${n(c.paidThisMonth)>0 ? 'var(--accent)' : '#aaa'}" onclick="markAsPaid('${c.id}')">PAY</button>
                <button class="btn-job" style="background:#8e8e93" onclick="editCust('${c.id}')">EDIT</button>
            </div>`;
...
window.toggleCleaned = (id) => { const c = db.customers.find(x => x.id === id); if(c) { c.cleaned = !c.cleaned; saveData(); renderWeekFence(); } };
window.markAsPaid = (id) => { const c = db.customers.find(x => x.id === id); if(c) { c.paidThisMonth = (n(c.paidThisMonth)>0) ? 0 : c.price; saveData(); renderWeekFence(); } };
