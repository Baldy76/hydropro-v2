...
window.renderStats = () => {
    ...
    container.innerHTML = `
        <div class="stats-hero" style="background:var(--card); padding:40px 20px; border-radius:40px; text-align:center; border:2px solid var(--accent); margin-bottom:20px;">
        ...
        <div style="background:var(--card); padding:25px; border-radius:35px; margin-bottom:20px;">
        ...
        <div style="background:var(--danger); color:white; padding:25px; border-radius:30px; text-align:center; font-weight:900; font-size:20px;">ARREARS: £${arrears.toFixed(2)}</div>`;
};
...
window.renderWeek = () => {
    ...
    db.customers.filter(c => c.week == curWeek).forEach(c => {
        ...
        div.innerHTML = `<div><strong style="font-size:21px;">${c.name} ${c.cleaned?'✅':''}</strong><br><small style="color:var(--accent); font-weight:700;">${c.houseNum} ${c.street}</small></div>
...
window.addExpense = () => {
    ...
    if(!d || a <= 0) return alert("Description and Amount required");
