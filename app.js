...
/* --- 📊 DASHBOARD REBUILD (v40.2) --- */
window.renderStats = () => {
    const container = document.getElementById('stats-dashboard-container'); if(!container) return;
    let targetIncome = 0, collected = 0, fuel = 0, gear = 0, food = 0, misc = 0;
    
    db.customers.forEach(c => { targetIncome += n(c.price); collected += n(c.paidThisMonth); });
    ...
    const netProfit = collected - totalSpend;
    const prog = targetIncome > 0 ? Math.min(Math.round((collected / targetIncome) * 100), 100) : 0;

    container.innerHTML = `
        <div class="ST-HERO">
            <small>Net Monthly Profit</small>
            <div style="color:${netProfit >= 0 ? 'var(--success)' : 'var(--danger)'}">£${netProfit.toFixed(2)}</div>
        </div>

        <div class="ST-GRID">
            <div class="ST-BUBBLE"><small>Total Paid In</small><strong>£${collected.toFixed(2)}</strong></div>
            <div class="ST-BUBBLE"><small>Total Spent</small><strong>£${totalSpend.toFixed(2)}</strong></div>
        </div>

        <div class="ST-PROG-PLATE">
            <div style="display:flex; justify-content:space-between; font-weight:900;">
                <span>ROUND PROGRESS</span><span>${prog}%</span>
            </div>
            <div class="ST-PROG-BAR"><div class="ST-PROG-FILL" style="width:${prog}%"></div></div>
            <div style="display:flex; justify-content:space-between; opacity:0.5; font-size:14px; font-weight:800;">
                <span>CASH: £${collected}</span><span>TARGET: £${targetIncome}</span>
            </div>
        </div>

        <div class="SU-CARD" style="margin-top:0; padding:25px;">
            <h4 style="margin:0 0 15px 0; opacity:0.4; text-transform:uppercase;">Spend Breakdown</h4>
            <div style="display:flex; justify-content:space-between; padding:12px 0; border-bottom:1px solid rgba(0,0,0,0.05); font-weight:800;"><span>⛽ Fuel</span><span>£${fuel.toFixed(2)}</span></div>
            <div style="display:flex; justify-content:space-between; padding:12px 0; border-bottom:1px solid rgba(0,0,0,0.05); font-weight:800;"><span>🛠️ Gear</span><span>£${gear.toFixed(2)}</span></div>
            <div style="display:flex; justify-content:space-between; padding:12px 0; font-weight:800;"><span>📦 Misc</span><span>£${misc.toFixed(2)}</span></div>
        </div>
    `;
};
...
window.nuclearReset = () => { if(confirm("☢️ DELETE EVERYTHING?")) { if(confirm("FINAL WARNING: All data will be wiped.")) { localStorage.removeItem(DB_KEY); location.reload(); } } };
window.completeCycle = () => { if(confirm("Clear month? Statuses reset but history kept.")) { db.customers.forEach(c => { c.cleaned = false; c.paidThisMonth = 0; }); db.expenses = []; saveData(); location.reload(); } };
